...
class GWplot:
	def __init__(self, idxs, chrs, scores, sid):
		...
		self.figfile = '/home/ales/integr/website/static/uploadify/wgplots/%s.png' % self.sid
...
def importAnnot():
	...
	annoloc = '/home/ales/integr/annot/*.txt'
	...
	for annot in glob.glob(annoloc):
		...
		con = lite.connect('/home/ales/integr/sqlite3/annotation.db')
		cur = con.cursor()
...
def prepareBackbone(interval, overlap):
	z=open('/home/ales/integr/data/genome_coor_hg19.txt', 'r')
...
def hgncImport(interval, overlap, numRegions):
	z=open('/home/ales/integr/data/Genes.txt', 'r')
...
def importAnnotStudy(studyFiles, study, studyTypes, studyNames, annoTypes, interval, overlap, cur, con):
	...
	cur.execute("ATTACH '/home/ales/integr/sqlite3/annotation.db' AS annotation")
...
class HelloWorld:
	...
	def ping(self, **kw):
		...
		for study in range(0,numStudies):
			...
			if(np.sum(output>1)>0):
				mat[:,study]=output
				yield "It seems that values for importing are not in the form of P-Values. Importing as scores..."
			else:
				mat[:,study]=-np.log10(output)
				yield "It seems that values for importing are not in the form of P-Values. Importing as scores..."
		...
		rfile = '/home/ales/integr/coords.txt'
		
		cur.execute("SELECT chr,start,stop FROM Backbone")
		output = cur.fetchall()
		
		f = open(rfile, 'w')
		for out in output:
			print out
			f.write(out[0] + '\t' + out[1] + '\t' + out[2] + '\n') 
		f.close()
