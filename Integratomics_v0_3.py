...
class HelloWorld:
	...
	def ping(self, **kw):
		...
		yield "This annotation is %s" % annoType
		all=[]
		...
		# 
		z=open('/Users/alesmaver/Desktop/data/dbSNP137.txt', 'r')
		...
		for j in range(len(chrLengths)):
			yield "Chromosome length %s <br />" % chrLengths[j]
		...
		for study in range(0,numStudies):
			...
			if annoTypes[study] == "SNP":
				...
				sql3 = "SELECT chr, start, stop, score FROM annotation.%s LEFT OUTER JOIN %s ON annotation.%s.name = %s.name" % (annoTypes[study], studyNames[study], annoTypes[study], studyNames[study])
				cur.execute(sql3)
				output = cur.fetchall()
				all = []
				for cols in output:
			...
			#Simulate data from three studies
			mat[:,study]=output
